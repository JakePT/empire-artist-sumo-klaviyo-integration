<?php
/**
 * This class is a copy of the pluggable SUMO Reward System plugin class defined
 * in the plugin's includes/frontend/tab/class-rs-fprsmessage-frontend.php file
 * as of version 24.8.
 *
 * It is 'plugged' to modify the output of the Reward Points table to:
 *
 * * Prominently display the total number of points earned.
 * * Better optimise the display of the table for mobile.
 *
 * The latter is achieved by replacing
 * includes/frontend/views/class-rs-frontend-my-reward-table.php with a file from
 * this plugin.
 *
 * The lines that have been chaned or added are marked with a CHANGED comment.
 *
...
class RSFunctionForMessage {
	...
	public static function reward_log_table( $TableData ) {
		...
		$EarnedPoints    = $PointData->total_earned_points() ; // CHANGED
		...
		if ( $TableData[ 'points_label_position' ] == '1' ) {
			echo "<h4 class=my_reward_total> " . $TableData['label_earned_points'] . ": " . $EarnedPoints . "<br>" . $TableData[ 'total_points_label' ] . " " . $AvailablePoints . " " . $msg . "</h4>" ; // CHANGED
		} else {
			echo "<h4 class=my_reward_total> " . $EarnedPoints . " " . $TableData['label_earned_points'] . "<br>" . $AvailablePoints . " " . $msg . $TableData[ 'total_points_label' ] . "</h4>" ; // CHANGED
		}
		...
		include EmpireArtist\KlaviyoSumo\get_plugin_file_path( 'templates/my-reward-table.php' ); // CHANGED
